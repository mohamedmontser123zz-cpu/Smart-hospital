...
def predict_patient(
    ...
    # Sanitize input values (fallback for missing/uncalibrated sensors)
    if temperature is None or float(temperature) <= 25.0:
        temperature = 36.6
    else:
        temperature = float(temperature)

    if heart_rate is None or float(heart_rate) <= 0:
        heart_rate = 75.0
    else:
        heart_rate = float(heart_rate)

    if spo2 is None or float(spo2) <= 0:
        spo2 = 98.0
    else:
        spo2 = float(spo2)

    if medex is None or float(medex) < 0:
        medex = 35.0
    else:
        medex = float(medex)
    ...
    if heart_rate > 100:
        reasoning.append(
            f"Elevated Heart Rate ({int(heart_rate)} BPM)"
        )
    elif heart_rate < 60 and heart_rate > 0:
        reasoning.append(
            f"Low Heart Rate ({int(heart_rate)} BPM)"
        )

    if spo2 < 94 and spo2 > 0:
        reasoning.append(
            f"Reduced Oxygen Saturation ({int(spo2)}%)"
        )
    ...
    elif temperature <= 35.0 and temperature > 25.0:
        reasoning.append(
    ...
    if medex >= 60:
        reasoning.append(
            f"High Stress / Medex Level ({int(medex)})"
        )
    elif medex <= 20 and medex > 0:
        reasoning.append(
            f"Low Stress / Medex Level ({int(medex)})"
        )

    if len(reasoning) == 0:
        reasoning.append("All vital signs within normal physiological ranges.")
            # ======================================================
    # Top 3 Predictions
    # ======================================================
